...
bool World::TryMergeRandomGroup()
{
    ...
    if (GenerateRandf() < 0.2)
    {
        ...
        // 保存被合并组的 ID，以免在 delete 之后访问已释放对象
        size_t smallerID = groups[smallerIndex]->GetGroupID();
        size_t largerID = groups[largerIndex]->GetGroupID();
        *(groups[smallerIndex]) += *(groups[largerIndex]);
        ...
        count--;
        cout << "--- 组 " << smallerID << " 和组 " << largerID << " 被合并为组 " << groups[smallerIndex]->GetGroupID()  << " ---" << endl;
        return true;
    }
    return false;
}
